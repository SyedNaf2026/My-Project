    ...
    this.auth.login(this.form.value).subscribe({
      next: (res) => {
        ...
        if (res.success) {
          ...
          setTimeout(() => this.toast.success(`Welcome back, ${res.data.fullName}!`), 0);
          const role = res.data.role;
          if (role === 'Admin') this.router.navigate(['/admin/dashboard']);
          else if (role === 'QuizCreator') this.router.navigate(['/creator/my-quizzes']);
          else this.router.navigate(['/taker/browse']);
        } else {
          setTimeout(() => this.toast.error(res.message || 'Invalid credentials.'), 0);
        }
      },
      error: (err) => {
        this.loading = false;
  ...
  submitForgot(): void {
    ...
    this.auth.resetPassword(this.forgotEmail.trim(), this.forgotNewPassword).subscribe({
      next: (res) => {
        ...
        this.cdr.detectChanges();
        if (res.success) {
          setTimeout(() => this.toast.success('Password reset! Please log in.'), 0);
